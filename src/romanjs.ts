type value_option = string | number;
type convert_option = 0 | 1 | 2;

function romanList(): any[] {
  const roman_letter: string[] = [
    "I",
    "II",
    "III",
    "IV",
 ...
 */
export class romanjs {
  private v1: value_option;
  private convert_type: convert_option;
  ...
  constructor(v1: value_option = __ZERO__) {
    this.v1 = v1;
   ...
   * roman range I to _M_M_M_C_M_X_C_I_XCMXCIX
   */
  public setValue(v1: value_option = __ZERO__): this {
    this.v1 = v1;
    this.convert_type = __UNKNOWN__;
    return this;
  }
  ...
  private numberBreakDown(v: value_option): number[] {
    let num: string = `${v}`;
    let arr: number[] = [];
  ...
  private detailNumberBreakDown(value: number): number[] {
    let arr: number[] = [];
    ...
    if (this.convert_type === __ROMAN__) {
      try {
        if (!this.isNumber(this.v1)) {
          ...
          let rawRomanNumber: any[] = [];
          let result: string = "";
          rawRomanNumber = this.numberBreakDown(this.v1).reduce(
            (p: number[], c: number) => {
              if (c > 1000 && c < 3999) {
          ...
          result = rawRomanNumber.reduce((p: string, c: number) => {
            __GENROMAN__.map((a) => {
              if (a.number == c) {
                return (p += a.roman);
              }
      ...
      try {
        ...
        let romanLetter: any = this.v1;
        let nonDashValue = __GENROMAN__.filter(
          (a) => a.roman.charAt(0) !== "_"
        );
        ...
        for (let i = 0; i < 7; i++) {
          ...
          if (romanLetter.charAt(index) === "_") {
            filterList = dashValue
              .filter(
                (a) => a.roman.charAt(0) === romanLetter.charAt(index + 1)
              )
              .map((a) => {
                return {
                  roman: `_${a.roman}`,
            ...
            filterList = nonDashValue.filter(
              (a) => a.roman.charAt(0) === romanLetter.charAt(index)
            );
            filterList.reverse();
